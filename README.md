...
- **Адаптивный интерфейс** — удобно на телефонах, планшетах и десктопах: на узких экранах показывается либо список чатов, либо окно чата с кнопкой «Назад»; учтены безопасные зоны (вырезы, жесты).
...
**Идеи по развитию** — в файле [IMPROVEMENTS.md](IMPROVEMENTS.md): уведомления, E2E, голосовые, PWA, поиск и др.

---

## Как выдать суперадмина

Суперадмин может заходить в админ-панель (кнопка **Админ** в шапке), смотреть список пользователей и чатов, жалобы и **выдавать/забирать** права суперадмина у других пользователей.

**Способ 1 — через переменные окружения (удобно при первом запуске)**

В `server/.env` или в настройках сервера задай одну из переменных и **перезапусти** сервер:

- **По id пользователя:**  
  `SUPERADMIN_USER_IDS=1`  
  или несколько через запятую: `SUPERADMIN_USER_IDS=1,5,12`
- **По логину:**  
  `SUPERADMIN_USERNAME=admin`  
  (логин в нижнем регистре)

При старте сервера эти пользователи получат флаг суперадмина в БД. После этого переменные можно убрать — права уже сохранены в базе.

**Способ 2 — через админ-панель**

Если уже есть хотя бы один суперадмин (например, выданный через способ 1), он может зайти в **Админ** → вкладка **Пользователи** → включить галочку **суперадмин** у любого пользователя.

**Способ 3 — скрипт (без установки sqlite3)**

В проекте уже есть Node и база. Запусти из **корня проекта** (папка `! dibil`):

```bash
node server/scripts/set-superadmin.js 1
```

Здесь `1` — это **id** пользователя (посмотри в БД или сделай по логину):

```bash
node server/scripts/set-superadmin.js admin
```

Скрипт сам найдёт `server/data/messenger.db`, обновит пользователя и выведет, кому выдан суперадмин. Логин можно писать в любом регистре (hope, Hope).

Узнать id и логины пользователей:
```bash
node server/scripts/list-users.js
```

**Способ 4 — напрямую в БД (если установлен sqlite3)**

```bash
sqlite3 server/data/messenger.db "UPDATE users SET is_superadmin=1 WHERE id=1;"
```

---

## После деплоя: что сохраняется

- **Фронт (GitHub Pages):** при каждом новом деплое подставляется свежая сборка. Токен и тема хранятся в **localStorage** в браузере — после обновления сайта пользователь остаётся в той же сессии, тема не сбрасывается.
- **Бэкенд (Render free):** при «засыпании» сервера файловая система сбрасывается: **SQLite и папка uploads не сохраняются** между перезапусками. Для постоянного хранения данных нужен внешний сервис БД (например PostgreSQL на Render) или VPS с постоянным диском. Для тестов текущей схемы хватает.
